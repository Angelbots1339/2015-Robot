 ...
 *
 */
public class PIDDriveWinchBin extends CommandBase {
	
	Joystick stick;
	double control;
	boolean up;

    public PIDDriveWinchBin() {
    ...
    protected void initialize() {
		...
		PIDElevator.setSetpoint(PIDElevator.groundValueListBin[1]);
    ...
    protected void execute() {
    	...
    	if(PIDElevator.isOnTarget()){
    		...
    		if(oi.getUpBtn() || oi.getDownBtn()){
        		...
        		if(up){
        			if(PIDElevator.currentStep == 1){
            			PIDElevator.setSetpoint(PIDElevator.groundValueListBin[2]);
